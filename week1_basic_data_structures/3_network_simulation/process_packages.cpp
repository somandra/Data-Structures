...
public:
    ...
    Response Process(const Request &request) {
        ...
        //std::cout<<finish_time_.size()<<' '<<size_<<"\n";
        if (finish_time_.size() < size_)
        {
            if(finish_time_.size() == 0){
                finish_time_.push(request.arrival_time+request.process_time);
                //std::cout<<"Arrival Time- "<<request.arrival_time<<"\n";
                return Response(false, request.arrival_time);
            } else {
                int start_time = request.arrival_time;
                if(finish_time_.back() > start_time){
                    start_time = finish_time_.back();
                } else if (finish_time_.back() == start_time)
                {
                    start_time = finish_time_.back()+1;
                }
